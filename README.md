...
* **디자인 자유도:** Tailwind CSS를 활용해 프로젝트의 '고급스러운 다크 네이비' 테마를 그리드 내부까지 100% 일관되게 적용했습니다.
...
* **인메모리 Job 관리:** 과제 환경의 단순함을 위해 별도의 메시지 브로커 없이 단일 서버 내 `ConcurrentHashMap`으로 비동기 작업을 관리하도록 타협했습니다. 이는 인프라 오버헤드를 줄이는 대신 서버 재시작 시 작업 유실이라는 트레이드오프를 수용한 결과입니다.
...
실제 프로덕션 환경에서 트래픽과 데이터가 급증할 경우를 대비한 PaaS 기반 고도화 전략입니다.
1. **Azure Functions / AWS Lambda:** 대량의 엑셀 파일 생성 및 복잡한 세무 통계 로직을 서버리스 워커로 분리하여 API 서버의 자원을 보호합니다.
2. **Managed Message Queue:** 현재의 내장 비동기 로직을 **Azure Service Bus** 또는 **AWS SQS**로 전환하여 작업의 영속성과 재시도 메커니즘을 구축합니다.
3. **Streaming Storage:** Export 시 데이터를 한 줄씩 읽어 **Cloud Storage**에 즉시 스트리밍 Write 함으로써 서버 메모리(RAM) 점유율을 최소화합니다.
